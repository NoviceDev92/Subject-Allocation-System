...
if students_file and subjects_file:
    ...
    if st.button("Run Allocation Algorithm"):
        st.session_state.allocated_df = allocation_engine.allocate(students_df, subjects_df)
    ...
    if st.session_state.allocated_df is not None:
        ...
        with tab1:
            ...
            csv_data = data_handler.export_data(allocated_df)
        ...
        with tab2:
            ...
            if search_id:
                student_data = data_handler.get_student_info(allocated_df, search_id)
        ...
        with tab3:
            ...
            if selected_subject:
                subject_data = data_handler.get_subject_students(allocated_df, selected_subject)
                st.metric("Total Enrolled", len(subject_data))
                st.dataframe(subject_data, use_container_width=True)

        with tab4:
            dept_mapping = data_handler.get_department_mapping(allocated_df)
        ...
        with tab5:
            ...
            with col1:
                st.plotly_chart(visualizer.plot_allocation_status(allocated_df), use_container_width=True)
            
            with col2:
                st.plotly_chart(visualizer.plot_subject_popularity(allocated_df), use_container_width=True)
                
            st.plotly_chart(visualizer.plot_department_distribution(allocated_df), use_container_width=True)
